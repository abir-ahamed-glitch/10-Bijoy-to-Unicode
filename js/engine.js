/**
 * Bijoy & Unicode Conversion Engine
 * Pure functions only — safe to import from Web Worker or main thread.
 * Auto-extracted from index.html. Regenerate with: node extract-engine.cjs
 */
...
const ENGLISH_SUFFIXES = /^(?:tion|sion|ment|ness|ity|ence|ance|ery|ry|ist|ism|able|ible|ous|ive|al|ic|ical|ful|less|ly|ed|er|or|ing|est|en|ize|ise|ify|ate|hood|ship|dom|craft|wright|wise|fold|ward|wards|like|less|ful|most|some|thing|body|one|self|selves)$/;
 ...
 */
const BIJOY_STRUCTURE_MULTI_KEYS = B2U_MULTI.map(x => x[0]).filter(k => k && k.length >= 2);
...
function isEnglishWord(word) {
  ...
  if (baseStem.length >= 2 && !BIJOY_BLACKLIST.has(baseStem) && (COMMON_ENG.has(baseStem) || EXTRA_ENG.has(baseStem))) return true;
...
function protectEnglishTokens(text) {
  ...
  //    Examples: "People & Society", "Water / Marine Sciences", "Bangladesh-India"
  //    Only protects when surrounding context looks English (no Bengali in the same run).
  //    Match: Optional 3+ words separated by & / - (with spaces). The first word must
  //    be title case or all-caps to avoid catching Bijoy punctuation.
  r = r.replace(/([A-Z][A-Za-z]+(?:[''][a-zA-Z]+)?(?:\s+(?:&|\/|-)\s+[A-Z][A-Za-z]+){1,5}(?:\s+(?:&|\/|-)\s+[A-Z][A-Za-z]+){0,5})/g, (m) => {
    // Only protect if every word in the phrase is English (or Title Case)
    const words = m.split(/\s+(?:&|\/|-)\s+/);
    if (words.every(w => isEnglishWord(w.trim()))) {
      return addProt(m);
    }
    return m;
  });

  // 5b. Hyphenated or slash-connected lowercase English compounds:
  //     "water-borne", "long-term", "sci-fi"
  r = r.replace(/([a-z]+(?:[-/][a-z]+){1,3})/g, (m) => {
    if (m.length < 5) return m;
    const parts = m.split(/[-/]/);
    if (parts.every(p => p.length >= 2 && (isEnglishWord(p) || EXTRA_ENG.has(p)))) {
      return addProt(m);
    }
    return m;
  });

  // 6. Tokenize remaining Latin text blocks
  r = r.replace(/(?<![A-Za-z0-9_\u0080-\uFFFF])([A-Za-z0-9]+(?:['’][a-zA-Z]+|\.)?)(?![A-Za-z0-9_\u0080-\uFFFF])/g, (token) => {
...
function bijoyToUnicodeSingleLine(line) {
  ...
  // Step 2: Multi-Char pre-pass (Longest matches first, e.g. cvÛzwjwc -> পাণ্ডুলিপি, AuZi -> আঁতর, Authorship-‡qi -> Authorship-এর)
  const multiMap = [];
  ...
  // Step 1a: SutonnyMJ normalization
  r = r.replace(/&,/g, '~');
  const bijoyCons = '[K-Z_`a-z\u00b2\u00b0\u00b1\u00b3\u00b5\u00b6\u00b7\u00b8\u00bb\u00bc\u00bd\u00be\u00c0\u00c1\u00c2\u00c3\u00c4\u00c5\u00c6\u00c7\u00c8\u00c9\u00ca\u00cb\u00cc\u00ce\u00cf\u00d7\u00d8\u00d9\u00da\u00db\u00dc\u00de\u00df\u00e0\u00e1\u00e2\u00e3\u00e4\u00e5\u00e7\u00e9\u00ea\u00eb\u00ec\u00ed\u00ee\u00ef\u00f0\u00f1\u00f2\u00f3\u00f4\u00f5\u00f6\u00f7\u00f8\u00f9\u00fa\u00fb\u00fc\u00fd\u00fe\u00af\02c6\00af\2014\00af\2018\00af\00bf\00af\2039\00af\00fa\00af\00a7\00afc\00afb\00afl\00af\00f8]';
  const bijoyVowelKars = '[vwxyz~\u00e6\201e\2021\2020\2030\02c6\0160u]';
  r = r.replace(new RegExp(`(${bijoyCons})(?<!${bijoyVowelKars}),(?=[A-Za-z0-9])`, 'g'), '$1~');
  r = r.replace(/([A-Za-z0-9_`~\u00e6\201e\2021\2020\2030\02c6\u0080-\u00FF])uv/g, '$1vu');
  r = r.replace(/\u201A/g, '\u09C2').replace(/\u201E/g, '\u09C3');
  r = r.replace(/\u2021v/g, '\u09CB').replace(/\u2020v/g, '\u09CB');

  // Step 1b: Reph pre-pass
  const rephRx = new RegExp(`([\u2021\u2020w\u2030])?(${bijoyCons})\u00a9`, 'g');
  r = r.replace(rephRx, (_, v, c) => '\u00a9' + (v || '') + c);

  // Step 3: Single-char
  r = r.split('').map(c => B2U_SINGLE[c] ?? c).join('');

  // Step 4: Reorder left-side vowels using STRICT dynamicCons
  const dynamicCons = '(?:[ক-হৎড়ঢ়য়]|\\uE002[\\uE200-\\uE2FF]\\uE002)';
  const dynamicCluster = `${dynamicCons}(?:${BN_HAS}${dynamicCons})*`;
  r = r.replace(new RegExp(`(${LEFT_V})(${dynamicCluster})`,'g'), (_, v, cl) => cl + v);
  ...
  // r = r.replace(/([ািীুূৃৄোৌ])্(?=[ক-হ])/g, '$1');
